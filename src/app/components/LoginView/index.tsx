...
const LoginView = (props: { styles: any, navigation: any }) => {
    ...
    const register = () => {
        if (refreshToken && username && firstName && lastName && firstName.length > 0 && lastName.length > 0) {
    ...
    return (
        <SafeAreaView style={props.styles.container}>
            ...
            {(!refreshToken) &&
                <WebView onMessage={handleSubmitBtn} ref={ref => (webView = ref)} source={{ uri: uri }} onNavigationStateChange={onNavigationStateChange} javaScriptEnabled={true} domStorageEnabled={true} startInLoadingState={false} style={{ flex: 1, marginTop: 20 }} />
            }
